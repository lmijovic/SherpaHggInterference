...
double DipoleSplitting_Base::Vijfc(int type,int mode)
{
  if (m_subtype==1) {
    switch (type) {
    case 1:
...
double DipoleSplitting_Base::Vif(int type,int mode)
{
  ...
  case 1:
  case 2:
    return Vijf(1,mode)+Vijfc(1,mode);
  case 3:
  case 4:
    return Vijf(4,mode)+Vijfc(4,mode)+m_nf*CSC.TR/CSC.CA*(Vijf(3,mode)+Vijfc(3,mode));
  }
  return 0.;
}
