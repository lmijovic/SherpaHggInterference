...
double Single_Virtual_Correction::DSigma(const ATOOLS::Vec4D_Vector &_moms,bool lookup,const int mode)
{
  ...
  if (!lookup && m_user_bvimode!=0) {
    ...
    if (disc>dabs(m_bsum)+dabs(m_isum)) {
      ...
      wgt=sum/dabs(m_vsum);
      m_dsweight=sum/(sum-((m_user_bvimode&1)?dabs(m_bsum):0.0));
    }
  ...
  if (!lookup && (m_user_bvimode&8)) {
    double sum(dabs(m_bsum)+dabs(m_isum)+dabs(m_vsum)), disc(sum*ran->Get());
    if (disc>dabs(m_bsum)) {
      p_partner->m_bvimode=6;
      m_dsweight=wgt=sum/(sum-dabs(m_bsum));
    }
    else {
      p_partner->m_bvimode=1;
      wgt=sum/dabs(m_bsum);
...
//        w *= -m_x1*log(eta1);
  ...
  if (m_checkborn &&
    ...
    msg_Out()<<"Virtual: OLE = "<<(p_loopme->Mode()?p_loopme->ME_Finite()/p_dsij[0][0]:p_loopme->ME_Finite())<<std::endl;
    msg->SetPrecision(6);
  }
  double kfactor1((m_bvimode&1)?KFactor(1):1.0);
  double kfactor(m_lastki=m_lastk=(m_bvimode&6)?KFactor():1.0);
