...
(Cluster_Amplitude *const ampl,const bool &soft)
{
  ...
  for (;campl;campl=campl->Prev()) {
    ...
    if (campl->NLO()&8) {
      for (Cluster_Amplitude *nampl(campl);
	   nampl;nampl=nampl->Next())
	nampl->SetNLO(nampl->NLO()&~1);
      for (Cluster_Amplitude *pampl(campl);
	   pampl;pampl=pampl->Prev())
	pampl->SetNLO(pampl->NLO()&~1);
    }
	...
	(*campl,ic,jc,kc,lij->Flav(),
	 lampl->Kin(),type,1|(swap?2:0),ws,mu2);
...
void Dire::RecoCheck(Amplitude *const a,int swap) const
{
  ...
  double ws, mu2;
  Splitting s=p_clus->KT2
    (*ampl,ic,jc,kc,a->Split().p_c->Flav(),
     a->Split().m_kin,a->Split().m_type,1|(swap?2:0),ws,mu2);
  ampl->Delete();
