...
int Shower::Evolve(Amplitude &a,double &w,unsigned int &nem)
{
  ...
  for (Splitting s(GeneratePoint(a,t));
	...
	s.m_kfac=0;
	s.m_t1=ampl->MuR2();
	double ref=s.p_sk->Value(s);
	s.m_clu=0;
	s.m_kfac=m_kfac;
	double cur=s.p_sk->Value(s);
	if (ref>cur) m_weight*=ref/cur;
	if (ran->Get()<ref/cur) {
	  a.Remove(s.p_n);
	...
	return 1;
      ...
      if (!(ampl->NLO()&1) || ampl->NLO()&16) return stat;
      msg_Debugging()<<"skip veto, w = "<<1.0/ampl->LKF()<<"\n";
      m_weight*=1.0/ampl->LKF();
      a.Remove(s.p_n);
      s.p_c->SetFlav(s.p_sk->LF()->Flav(0));
      for (size_t i(0);i<a.size();++i) a[i]->Restore();
      for (Cluster_Amplitude *campl(ampl);
	   campl;campl=campl->Prev()) campl->SetLKF(1.0);
      ampl->SetNLO(ampl->NLO()&~1);
      continue;
    }
    if (stat<0) continue;
    double cw(1.0);
