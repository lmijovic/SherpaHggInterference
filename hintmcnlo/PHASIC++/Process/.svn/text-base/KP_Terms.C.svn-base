...
KP_Terms::KP_Terms(Process_Base *const proc,const int mode):
  m_imode(6), m_subtype(0),
  p_proc(proc), p_nlomc(NULL),
  p_flkern(new Flavour_Kernels()), p_masskern(NULL)
{
  SetNC(3.0);
  m_subtype=ToType<int>(rpa->gen.Variable("NLO_SUBTRACTION_SCHEME"));
  p_flkern->SetSubType(m_subtype);
 ...
 const double &eta0,const double &eta1,const double &weight)
{
  ...
  if (sa) {
    ...
    double t=0., tc=0.;
    if (!m_massive) {
      int atype=m_flavs[m_plist[0]].IntSpin();
      for (size_t i=pls;i<m_plist.size();i++) {
	int itype=m_flavs[m_plist[i]].IntSpin();
	t+= m_dsij[0][i]*p_flkern->ft(itype);
	tc+= m_dsij[0][i]*(p_flkern->t2c(itype)+p_flkern->t2c(atype));
      }
      m_kpca[type*2-2]+=t*(-w*p_flkern->t1(x0)+p_flkern->t2()-p_flkern->t4(eta0))+tc;
    ...
    if (sb) {
      if(!m_massive){
	m_kpca[type*2-2]+=m_dsij[0][1]*p_flkern->t2c(type);
  ...
  if (sb) {
    ...
    double t=0., tc=0.;
    if (!m_massive) {
      int btype=m_flavs[m_plist[pls-1]].IntSpin();
      for (size_t i=pls;i<m_plist.size();i++) {
	int itype=m_flavs[m_plist[i]].IntSpin();
	t+= m_dsij[pls-1][i]*p_flkern->ft(itype);
	tc+= m_dsij[pls-1][i]*(p_flkern->t2c(itype)+p_flkern->t2c(btype));
      }
      m_kpcb[type*2-2]+=t*(-w*p_flkern->t1(x1)+p_flkern->t2()-p_flkern->t4(eta1))+tc;
    ...
    if (sa) {
      if (!m_massive){
	m_kpcb[type*2-2]+=m_dsij[0][1]*p_flkern->t2c(type);
