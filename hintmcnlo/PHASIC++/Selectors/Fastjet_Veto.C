...
namespace PHASIC {
  ...
  public:
    Fastjet_Veto(int nin, int nout,ATOOLS::Flavour * fl,std::string algo,
...
Fastjet_Veto::Fastjet_Veto(int nin, int nout,ATOOLS::Flavour * fl, std::string algo,
			       double ptmin, double etmin, double dr, double f, double eta, double y, int nn, int nb, int nb2) :
...
bool Fastjet_Veto::NoJetTrigger(const Vec4D_Vector &p)
{
  return true;
}
...
bool Fastjet_Veto::JetTrigger(const Vec4D_Vector &p,
				ATOOLS::NLO_subevtlist *const subs)
{
  std::vector<fastjet::PseudoJet> input,jets;
...
operator()(const Selector_Key &key) const
{
  ...
  Fastjet_Veto *jf(new Fastjet_Veto(key.p_proc->NIn(),key.p_proc->NOut(),
